...
from solver import FirstFit, FirstFitDescending, NextFit
from st_aggrid import AgGrid, GridOptionsBuilder, GridUpdateMode

st.header("Bin Packing Simulator")
placeholder = st.empty()
option = st.selectbox("Algorighm", ("Next Fit", "First Fit", "First Fit Descending"))
...
if "weight" not in st.session_state.keys() or make_weight_button:
    st.session_state.weight = pd.DataFrame(np.random.randint(low=1, high=bin_capacity, size=numbers_of_items), columns=["weight"], index=pd.RangeIndex(1, numbers_of_items + 1, name="item_id"))
